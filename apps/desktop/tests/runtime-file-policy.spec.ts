...
it('omits development artifacts while preserving executable modules, assets and license files', async () => {
  ...
  const retained = [
    ...
    'native-sample/build/Release/native_sample.node', 'native-sample/build/Release/runtime.dll',
...
it('applies package-specific rules inside scoped and nested dependency containers', () => {
  ...
  expect(desktopRuntimeFileExclusion('outer/node-pty/prebuilds/win32-arm64/conpty.node', windows)).toBeUndefined()
})
