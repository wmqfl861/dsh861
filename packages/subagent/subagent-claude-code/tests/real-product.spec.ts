...
describe('real Claude Agent SDK 0.3.269 and its distributed Claude Code 2.1.269 fixture', {
  timeout: 60_000,
}, () => {
  it('inherits host settings and sends the exact task and fake key to local Messages', async () => {
    ...
    expect(sdkPackage.version).toBe('0.3.269')
    expect(sdkPackage.claudeCodeVersion).toBe('2.1.269')
    expect(sdkPackage.optionalDependencies[platformPackage]).toBe('0.3.269')
    const version = await execFileAsync(claudeBin, ['--version'], {
      env: { ...process.env, ...harness.env },
    })
    expect(version.stdout.trim()).toBe('2.1.269 (Claude Code)')
    ...
    expect(initMessage?.claude_code_version).toBe('2.1.269')
