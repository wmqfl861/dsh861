...
Creation is one rollback-covered transaction: construct a private session, concrete agent, and scoped context; await optional setup with the context and Agent passed separately; enter both registries; announce `session/created` then `agent/created`; emit `agent/session-start`; only then start the driver. A caller creating a runtime child sets `options.parentAgent`; the caller Context separately owns the transaction and live handle. A setup throw, commit failure, or owner disposal rolls the transaction back without publishing either id. Teardown publishes the agent's shared completion before any reentrant step, closes admission, starts every tracked obligation — live handles and startup tasks — and reports the original failures only after all of them settle; an optional `teardown` hook supplies the synchronous `begin(agent, completion)` handover and the `beforeRelease(agent)` preparation point, and the machine's scope disposer is collected on the factory fiber so structural release joins the same settlement. Detach order is stop-and-drain, write-path close, scope unwind, agent detach, session detach, and every detach is bound to the exact entered object so a stale disposer cannot remove a later same-id replacement.
