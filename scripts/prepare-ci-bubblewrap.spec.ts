 ...
 *
 * Linux rework (run 35447649954): the POSIX parent PATH tail carries the
 * runner's real toolchain, so the missing-tool scenario withholds it
 * (`omitPosixParentPath`); POSIX resolves an absolute bash once through the
 * vitest parent, so no spawn ever searches the child PATH the dual-key
 * scenarios replace; and the stdin-mode tar stub records its call after
 * draining stdin, making the `dpkg-deb | tar` pipeline log order
 * deterministic (startup-time logging raced on Linux).
 */
...
interface ScenarioOptions {
  ...
  /**
   * POSIX-only: build a stub-only PATH with no parent tail, so an omitted
   * tool is genuinely absent. Linux CI runner parents carry real cc/ninja/
   * python3/pkg-config (run 35447649954: the missing-tool scenario received
   * status 0); no effect on win32, which never inherits parent path content.
   */
  readonly omitPosixParentPath?: boolean
}
...
const runScenario = (options: ScenarioOptions = {}): ScenarioResult => {
  ...
  const stubs: Record<string, string> = {
    ...
    tar: [
      ...
      '    # dpkg-deb stub must never write into a closed pipe read end. The',
      '    # call record is written after the drain, so a `dpkg-deb | tar`',
      '    # pipeline always logs upstream-first: the dpkg-deb stub records at',
      '    # its startup, this stub records at its stdin EOF. Startup-time',
      '    # logging raced the upstream record on Linux (CI run 35447649954:',
      '    # the deb section interleaved T1/FS1/FS2/T2 instead).',
      '    cat > /dev/null',
      '    printf \'%s\\n\' "tar $*" >> "$STUB_LOG"',
      '    case "$mode" in',
  ...
  const envInput: ScenarioEnvInput = {
    ...
    posixParentPath: options.omitPosixParentPath === true ? 'omit' : 'append',
  }
...
describe.skipIf(!bashUsable)('prepare-ci-bubblewrap.sh end to end (real script, stubbed external effects)', () => {
  ...
  it('fails loud when a build tool is missing, before any download', () => {
    // Stub-only PATH on POSIX: the Linux CI runner parent carries the real
    // toolchain (run 35447649954 let a real `ninja` satisfy the preflight and
    // the whole scenario succeed). Withholding the tail keeps the omission
    // genuine without touching the default POSIX append contract other
    // scenarios assert.
    const result = runScenario({ omitTool: 'ninja', omitPosixParentPath: true })
    expect(result.status).toBe(1)
...
describe('scenario child environment construction (path-variable uniqueness)', () => {
  ...
  it('posix omit: builds a stub-only PATH so a deleted tool stays genuinely absent', () => {
    const env = buildScenarioChildEnv({
      pathEntries: [stubBin],
      parentPathEntries: [{ name: 'PATH', value: posixParentPath }],
      testVars: {},
      platform: 'linux',
      posixParentPath: 'omit',
    })
    expect(Object.keys(env).filter(name => name.toLowerCase() === 'path')).toEqual(['PATH'])
    expect(env.PATH).toBe(stubBin)
  })
...
describe.skipIf(!bashUsable)('prepare-ci-bubblewrap.sh stub-source identity and launch environment (W02)', () => {
  ...
  it('executes through the resolved absolute Bash with a stub-first PATH and records the source evidence', () => {
    ...
    if (process.platform === 'win32') {
      ...
      // POSIX resolves and keeps an absolute bash: the dual-key scenarios
      // replace the PATH a bare name would be searched through.
      expect(result.bashCommand).toMatch(/^\/.*\/bash$/)
    }
    expect(result.status).toBe(0)
  ...
  /**
   * Duplicate-key parent fixture. POSIX appends the exact-`PATH` entry, so
   * its value must be a functional POSIX tail (the script still needs the
   * coreutils it provides) while the differently-cased `Path` entry stays
   * Windows-form garbage — ignored on POSIX, dropped on win32. run
   * 35447649954 failed both scenarios with `probe status null` because the
   * old fixture left the appended tail without any bash or coreutils
   * directory.
   */
  const dualKeyParentEntries = (first: 'Path' | 'PATH'): readonly RawEnvEntry[] => {
    const pathEntry: RawEnvEntry = { name: 'Path', value: 'C:\\Windows\\System32;C:\\Program Files\\dotnet' }
    const posixTailEntry: RawEnvEntry = { name: 'PATH', value: '/usr/bin:/bin' }
    return first === 'Path' ? [pathEntry, posixTailEntry] : [posixTailEntry, pathEntry]
  }

  it('runs unchanged when the parent environment carries both Path and PATH variants (Path first)', () => {
    const result = runScenario({ parentPathEntries: dualKeyParentEntries('Path') })
    expect(result.status).toBe(0)
    expect(result.unameResolution).toBe(result.expectedUnameResolution)
    expect(result.stdout).toContain('published private-build bubblewrap')
    expect(result.githubPathContent).toContain('/build')
