...
# The verbose build log must contain the real link command and it must prove
# the private libcap link in one of the two forms Meson emits for this
# configuration: the private search path plus the linker name (`-L<private>
# ... -lcap`), or the private static archive as a direct absolute operand —
# the form the real CI run 35445600344 linked with ([7/7] cc ...
# <private>/libcap.a), which carries no search-path ambiguity at all. Any
# other command — no private reference, or host paths only (-L/usr/lib...,
# a host libcap.a operand) — means the binary fell back to host development
