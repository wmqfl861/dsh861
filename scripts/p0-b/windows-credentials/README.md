...
[planner-entry.ts](planner-entry.ts) composes the projection, credential lease, process wrapper and integrity-checked Windows job helper. The entry reserves a fresh run root exclusively before creating children and writing `config.toml` with `wx`; an existing root, including a pre-existing link, is refused. Its parent directories and Windows ACL remain the trusted caller's responsibility. Empty approval references and a mismatched subject are rejected before materialization; nonempty reference strings do not authenticate an owner decision.

The optional `ownership` hook starts the CLI through the pinned gate launcher ([launch-gate.mjs](launch-gate.mjs)) when it provides `launch`: the launcher is spawned with the CLI's exact stdio, working directory and environment, joins the owned job through `assign()`, and creates the CLI only after `release()` confirms that membership, so the CLI's first code runs already inside the job and descendants it creates join the same job. The launcher never reads stdin and creates nothing before the go marker; assignment failure, late assignment after cancellation, or a cancelled run writes the abort marker and never creates the CLI, and the prompt is delivered only after confirmed membership on a live, uncancelled invocation. An ownership hook without `launch` spawns the CLI directly and keeps the residual gap that startup work can run before PID assignment. A gated start is launch wiring, not process-tree verification; `descendantState` stays `NOT_VERIFIED`.

[job-owner.ps1](job-owner.ps1) holds the explicitly owned job without breakaway flags, so every process a member creates joins the job. [windows-job-owner.ts](windows-job-owner.ts) verifies the pinned PowerShell, helper and gate-launcher hashes at creation and accepts only bounded, operation-matched JSON replies with consistent success fields and nonnegative process counts. Protocol or transport failure remains latched; later disposal cannot turn failure into success. `dispose()` resolves only after a valid acknowledgement and observed clean helper close within the reply/close deadline. A termination acknowledgement is a request result, not proof that all processes have exited.

The entry records `assigned`, `terminated`, `activeProcessesRemaining` and `disposed` after cleanup. Failed termination, a nonzero or unknown count, missing assignment, or failed disposal returns `PROJECTED_PLANNER_CLEANUP_BLOCKED` rather than completed/cancelled status. Invocation errors retain their cleanup receipt. An empty owned job does not cover a descendant created before assignment; `descendantState` stays `NOT_VERIFIED`. Process completion still does not certify a usable plan, model identity, ownership of every descendant or product acceptance.
...
The entry suite also runs only on Windows, using real PowerShell and synthetic processes to check termination within the assigned job and concurrent isolation. Its module-top-level descendant control and its delayed-assignment control pin the pre-launch containment boundary deterministically. The gated-launcher native results, the completed `PROCESS_INFORMATION` layout probe and the two local test-source repairs are recorded in the [r13 Windows successor receipt](../../../development/remediation/2026-09-10/planner-gate-r13/verification.json). Historical results and the pinned-CLI keyless parsing evidence remain in the [r12 receipt](../../../development/remediation/2026-09-10/planner-entry-r12/verification.json); fixtures that create descendants only after receiving input do not establish containment before startup.

[ownership-failures.test.mjs](ownership-failures.test.mjs) loads the actual TypeScript sources in isolated VM modules with explicitly simulated processes, Windows platform and file boundaries. It checks failed/late assignment, invalid replies, bounded protocol and failed-cleanup states. It runs no PowerShell or credential store and is not native Windows or authorization evidence. The [r13 receipt](../../../development/remediation/2026-09-10/ownership-failure-r13/verification.json) records the execution environment, initial control failures and unexecuted checks.
