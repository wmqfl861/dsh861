...
export interface ScenarioEnvInput {
  ...
  /**
   * POSIX parent-PATH handling: `append` (default) appends the exact-`PATH`
   * parent entries after the explicit ones — the contract every ordinary
   * scenario relies on. `omit` builds a stub-only PATH for scenarios that
   * must prove a tool is genuinely absent: Linux CI runner parents carry
   * real cc/ninja/python3/pkg-config on their PATH (run 35447649954), so a
   * deleted stub stays shadowed unless the tail is withheld. No effect on
   * win32, where parent path content is never inherited at all.
   */
  readonly posixParentPath?: 'append' | 'omit'
}

/** Resolved launch identity for the spec's Bash child process. */
export interface SpecBash {
  /** Absolute Bash path on every platform (`C:\...\bash.exe` on win32, `/usr/bin/bash`-style on POSIX). */
...
/**
 * Resolves the Bash the scenario children will execute, always as an
 * absolute path. On POSIX the absolute path is resolved once through the
 * vitest parent (`command -v bash`) and kept: spawning a bare `bash` name
 * would make libuv search the CHILD environment's PATH, which the
 * dual-key scenarios deliberately replace — the POSIX dual of the Windows
 * bare-name fragility (run 35447649954: both dual-key scenarios died with
 * `probe status null` because no bash directory remained reachable). win32
 * probes absolute Git Bash candidates (fixed locations, then
 * `%LOCALAPPDATA%`, then `where.exe` output) and accepts the first one that
 * answers a `--noprofile --norc` probe.
...
export const resolveSpecBash = (): SpecBash => {
  if (process.platform !== 'win32') {
    const resolved = spawnSync('bash', ['--noprofile', '--norc', '-c', 'command -v bash'], {
      encoding: 'utf8',
      timeout: 10_000,
    })
    ...
    return { command: 'bash', usrBinPosix: null, kind: 'unavailable' }
  }
 ...
 * parent `PATH` appended after them on other platforms, or omitted entirely
 * for the stub-only `posixParentPath: 'omit'` variant). No parent variable
 ...
 * @param input Ordered PATH entries, parent path entries, test vars, platform, POSIX tail mode.
...
export const buildScenarioChildEnv = (input: ScenarioEnvInput): NodeJS.ProcessEnv => {
  ...
  } else if (input.posixParentPath === 'omit') {
    env.PATH = input.pathEntries.join(':')
  } else {
