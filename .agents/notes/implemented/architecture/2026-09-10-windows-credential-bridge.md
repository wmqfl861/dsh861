...
The minimal caller entry composes that projection with the existing lease and wrapper: argv, environment and `CODEX_HOME` come only from the projection, and containment comes only from an explicitly owned Windows job object held by an integrity-checked helper. Node 26.4.0's own libuv job allows silent breakaway, measured on this host, so a wrapper's exit never reaches CLI descendants and ownership must be explicit. The owned job carries no breakaway flags; detached descendants and explicit `CREATE_BREAKAWAY_FROM_JOB` attempts stayed members in measurement, with the fixed input delivered only after assignment settles so descendants created on input receipt join the scope. Termination is explicit (`TerminateJobObject`) on cancel, dispose and owner death, with `KILL_ON_JOB_CLOSE` as backstop; the flag is set by whole-struct assignment because nested value-type field mutation is a silent no-op in PowerShell — a defect the owner-death fail-safe test caught after `SetInformationJobObject` reported success with the flag unset.
