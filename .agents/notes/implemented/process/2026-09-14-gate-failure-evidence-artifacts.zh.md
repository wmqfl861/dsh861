...
首批下载的 artifact 暴露两个归属缺陷（r31 整改记录 `development/remediation/2026-09-14/ci-evidence-r31/`）。consumers artifact 记录的是嵌套 `node-compat` aggregate 的通过结果而非外层 `ci-consumers` 失败：gate 子进程继承 `process.env`，嵌套 run-gates CLI 解析到同一个 `DSH_GATE_EVIDENCE_DIR`，先行导出并触发外层的拒绝非空目录。同时所有 artifact 的 `identity.git.headParent` 均为 `null`，因为 `git show --format=%P` 在工作流 depth-one checkout 上返回空，而存储的 commit 头部仍保有父链。

## Decision

- `run-gates.ts` 增加显式启用的证据导出：`DSH_GATE_EVIDENCE_DIR` 指定输出目录。未设置或为空即无文件、无行为变化；调度、gate 命令、`GateResult` 与 stdout/stderr 所有权不变。
- CLI 入口经 `claimGateEvidenceRequest` 解析请求：先运行只读解析器，仅当其返回 request 时才从自身 `process.env` 删除该开关。request 保有目录与身份元数据，外层导出仍写入原目录；gate 子进程——包括 `check:node-compat` 等嵌套 run-gates aggregate——继承的其余环境中没有目录开关，无法再抢先占用目标位置。只读解析器继续导出供嵌套内省与测试使用；claim 不删除目录、不放宽空目录要求、不动未设置或为空的开关。
...
- `identity.json` 记录 repository、PR 编号、run id、run attempt、job、aggregate、PR head/base、checkout 的 `github.sha`/`github.ref`、实际 Git HEAD 及其第一父提交（`headParent`）与完整父列表（`headParents`）、Node/pnpm/平台；PR head、checkout merge SHA 与真实 HEAD 保持分立字段。父提交来自 `git cat-file commit HEAD`，只解析第一个空行之前的 `parent ` 头部行，depth-one checkout 因此保住存储的父链，commit message 文本无法进入列表。只读取白名单内的非敏感 CI 变量——不 dump 环境、事件载荷或凭据存储。
...
失败的红项车道产出可逐字节复核的 artifact：身份、状态与日志均可校验——状态逐字复制自 `GateResult`（fail-fast 跳过保持 skipped、aborted 保持 failed），上传仅在 gate 步骤自身失败后运行，因此失败车道缺少 artifact 本身即是一项发现。artifact 现在始终归属失败的那条车道——外层 aggregate 在任何 gate 启动前认领目标位置，嵌套 aggregate 自身的结果无法再改写记录。`scripts/gate-evidence.spec.ts` 拥有导出行为（开关关闭、状态忠实、退出码保持、脱敏反向断言、截断可见、拒绝非空目录、防遍历文件名、manifest 复算、run/attempt/job 分立、真实子进程下的目标位置认领、depth-one 双父 fixture 的存储父链）；`ci-workflow.spec.ts` 固定三条车道的接线。
...
| 在调度器内按 gate 清除 `DSH_GATE_EVIDENCE_DIR` | 把环境所有权放进 `runGate`，而本变更不得触碰其调度与子环境组装；CLI 入口一次性认领覆盖全部后代 |
| `git fetch --unshallow` 或 `fetch-depth: 0` 恢复 `%P` | 下载工作流从不需要的历史；存储的 commit 头部本就保有父提交 |
