...
export function selectSessionLogById(logs: readonly PersistedSessionLog[], id: string): SelectedSessionLog {
  ...
  const [selected, ...duplicates] = identified.filter(entry => entry.header.id === id)
  if (selected === undefined) {
    const found = identified.map(entry => `${entry.log.source} (id ${entry.header.id})`).join(', ')
    throw new Error(`no persisted session log with id "${id}"; found: ${found === '' ? 'none' : found}`)
  }
  if (duplicates.length > 0) {
    throw new Error(`duplicate persisted session logs with id "${id}": ${selected.log.source}, ${duplicates.map(entry => entry.log.source).join(', ')}`)
  }
